...
#
# echo "Press a key to:"
# echo -e "[${GREEN}r${NOCOLOR}] - restart the Preview app on all connected devices"
# echo -e "[${GREEN}s${NOCOLOR}] - reset the Preview app on all connected devices"
# while true; do
#   # Read a single character from the input
#   read -n 1 key
#   # Check if the pressed key is 'q'
#   if [[ $key == "r" || $key == "s" ]]; then
#     echo "$key" >$mobile_pipe
#   fi
# done
