...
): Promise<RuleSetUpsertRow | null> {
  const rows = await sql
    .unsafe(ruleSetUpsertQuery, [args.id, args.userId, args.name])
    .values();
  if (rows.length !== 1) {
    return null;
  }
...
): Promise<RuleSetRoadTagsUpsertRow | null> {
  const rows = await sql
    .unsafe(ruleSetRoadTagsUpsertQuery, [
      args.userId,
      args.ruleSetId,
      args.tagKey,
      args.value,
...
export async function planCreate(
  sql: Sql,
  args: PlanCreateArgs,
): Promise<PlanCreateRow | null> {
  const rows = await sql
    .unsafe(planCreateQuery, [
      args.userId,
      args.id,
      args.name,
      args.startLat,
