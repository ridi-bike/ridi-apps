...
export function GeoMapStatic(props: GeoMapStaticProps) {
  const bbox = useMemo(() => {
    const features = turf.points(props.points.map(p => ([p.coords[1], p.coords[0]])));
  ...
  return (
    ...
    >
      {props.points.map((point) => (
        <GeoMapMarker key={`${point.coords[0]}-${point.coords[1]}`} lat={point.coords[0]} lon={point.coords[1]}>
          {point.icon}
        </GeoMapMarker>
      ))}
    </MapLibre>
