...
import { supabase, trpc, trpcClient } from "./supabase";
...
export const tracks$ = observable(
	synced<Tracks>({
		get: async (params) => {
		...
		set: async (params) => {
			if (params.changes.some(change => change.path.length !== 1 || change.path[0] !== "trackRequests")) {
				throw new Error("unsupported changes")
			}
			for (const change of params.changes) {
				const newTrackReq = (change.valueAtPath as TrackRequest[]).find(trRq => !(change.prevAtPath as TrackRequest[]).find(trReqPrev => trReqPrev.id === trRq.id))
		...
		subscribe: ({ refresh }) => {
			const channel = supabase.realtime.channel("track_requests").on(
				"postgres_changes",
				{
					event: "*",
					schema: "public",
				},
				(_payload) => { console.log("changes", { _payload }); refresh() },
			).subscribe()
